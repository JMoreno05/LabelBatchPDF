...
## checks the path of the application and confirms that the needed files are in that same spot
def readAppLocation():
    global programPath
...
## checks that requried files are there
def checkDependency():
    global programPath
...
## read configuration csv file to get paths to labels
...
## read the data from the csv generated by ui(MS Access)
## and store in array
...
## checks that paths in config file are valid
...
## check if the labels exist for that item
...
## records errors found in a csv that will open after run complete
def exportErrorCSV():
    global errors
...
## append the provided file x times to the destination file (idenpendent of which file it is; exp or barcode)
...
## opens the pdf and the error csv for visual output
## should also show errors in commandline
def endOfProgram():
    # print("eof")
